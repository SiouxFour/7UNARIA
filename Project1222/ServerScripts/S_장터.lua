...
function s_market:players()
	...
	if #list[1] <= 0 then
		unit.SendCenterLabel("<size=15><Color=Red>[장터]</color> 장터에 올라간 물건이 없습니다.</size>")
		return
	end
	...
	for i, v in ipairs(Server.players) do
		if v.id == target_playerID then
			...
			if unit.gameMoney < item[4] then
				unit.SendCenterLabel("<size=15><Color=Red>[장터]</color> 골드가 <Color=Yellow>" .. item[4]-unit.gameMoney .. "</color>Gold 부족합니다.</size>")
				s_market:target(v.id, v.name)
				return
			end
			...
			for j, y in ipairs(a) do
				if y.dataID == item[1] and #y.options == 0 and y.level == 0 then
					...
					v.unit.SetVar(var_fee, item[4]/20)
					v.unit.AddGameMoney(item[4]-v.unit.GetVar(var_fee))
					v.unit.SendCenterLabel("<size=15><Color=Red>[장터] </color><Color=Yellow>" .. Server.GetItem(item[1]).name .. "</color>를(을) 팔아<Color=Yellow>" .. item[4]-v.unit.GetVar(var_fee).. "</color>Gold획득![수수료:<color=#FFFF00>" .. v.unit.GetVar(var_fee).. "</color>Gold]</size>")
					v.unit.SendSay("<color=#FF0000>[장터] </color><color=#FFFF00>" .. Server.GetItem(item[1]).name .. "</color>를(을) 팔아<color=#FFFF00>" .. item[4]-v.unit.GetVar(var_fee).. "</color>Gold획득![수수료:<color=#FFFF00>" .. v.unit.GetVar(var_fee).. "</color>Gold]")
					s_market:target(v.id, v.name)
				break
				end
			end
